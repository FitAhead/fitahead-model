...
assets/models/            생성된 에셋 (커밋됨)
  male.glb  female.glb      스킨 + 모프 타겟 16종 (해부학 마네킹, 의상 없음)
...
반바지를 걷어냈습니다. 허리밴드 로프트 + 허벅지 튜브 **두 개의 별도 셸**이었고,
겹치기만 하는 두 셸은 경계에서 절대 깔끔하게 만나지 않습니다. 밑단이 커프를 지나는 자리에
턱이 생기거나 둘 사이에 구멍이 났고, 모프가 켜지면 허리밴드가 복부 지방 모프와 함께
부풀어서 기저귀처럼 보였습니다.

의상은 **하나의 연속 표면**이어야 합니다 — 허리에서 시작해 두 다리로 갈라지는 자체 토폴로지.
`polytube`로 사지 이음매를 없앤 것과 같은 종류의 작업이고, 지금은 캐릭터의 핵심 가치가
아니라서 미뤘습니다. 현재는 나체 해부 마네킹이고, 이건 모든 해부학 도판이 취하는 형태입니다.

> 앱 스토어·사용자 정서 측면에서 의상이 필요한지는 제품 판단입니다. 필요하면
> 위 방식으로 제대로 만들어야 하고, 지금 구조(별도 셸)로는 다시 같은 문제가 생깁니다.
...
- [ ] 손가락 — 현재 손은 손바닥 패들
- [ ] **의상** — 위 "의상이 없는 이유" 참고. 단일 연속 표면으로 재작업 필요
- [ ] 헤어 커스터마이징
